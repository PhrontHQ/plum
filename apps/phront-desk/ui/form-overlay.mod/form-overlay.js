...
exports.FormOverlay = DataEditor.specialize({

    constructor: {
        value: function FormOverlay() {
            this.super();

            this.application.addEventListener("openFormOverlay", this, false);
    ...
    handleInput: {
        ...
        value: function(event) {
            console.log("handleInput: event.target.value is "+event.target.value);
        }
    },
    ...
    loadRespondentQuestionnaireProperties: {
        value: function(respondentQuestionnaire) {

        }
    },

    createRespondentQuestionnaire: {
        value: function(questionnaire) {
            var respondentQuestionnaire = this.dataService.createDataObject(RespondentQuestionnaire);

            respondentQuestionnaire.originId = this.data.originId;
            respondentQuestionnaire.questionnaire = questionnaire;
    ...
    createRespondentQuestionnaireIfNeeded: {
        value: function(questionnaire) {
            var self = this;

            return new Promise(function(resolve, reject) {
                var respondentQuestionnaires = questionnaire.respondentQuestionnaires,
                respondentQuestionnairesPromise;

                if(respondentQuestionnaires === undefined) {
                    respondentQuestionnairesPromise = self.dataService.getObjectProperties(questionnaire, [ "respondentQuestionnaires"]);
                } else {
                    respondentQuestionnairesPromise = Promise.resolve(respondentQuestionnaires);
                }

                respondentQuestionnairesPromise
                .then((respondentQuestionnaires) => {
                    if(respondentQuestionnaires && respondentQuestionnaires.length === 0) {
                        Promise.resolve(null);
                    } else {
                        return respondentQuestionnairesPromise = self.dataService.getObjectsProperties(respondentQuestionnaires, [ "respondent", "questionnaire", "events"])
                    }
                })
                .then((respondentQuestionnaires) => {
                    if(respondentQuestionnaires) {
                        var patientEvent = self.data.event.children[0],
                        eventsCriteria = new Criteria().initWithExpression("eventsCriteria.has($.event)", {
                            event: patientEvent
                        }),
                        filteredRespondentQuestionnaires = respondentQuestionnaires.filter(practicePatientRelationshipTemplateCriteria.predicateFunction);

                        if(filteredRespondentQuestionnaires && filteredRespondentQuestionnaires.length === 1) {
                            resolve(filteredRespondentQuestionnaires[0])
                        } else {
                            resolve(self.createRespondentQuestionnaire(questionnaire));
                        }
                    } else {
                        //create
                        resolve(self.createRespondentQuestionnaire(questionnaire))
    ...
    createSupplementalHealthRespondentQuestionnaireIfNeeded: {
        value: function(supplementalHealthQuestionnaire) {
            return new Promise(function(resolve, reject) {
                resolve(null);
            });
        }
                ...
                .then((result) => {
                    if(result && result.length === 1) {
                        ...
                        .then(() => {
                            ...
                            var supplementalInformedConsentQuestionnaire = result[0],
                                supplementalInformedConsentRespondentQuestionnairePromise,
                                /*
                                    Right now the questionnaires are modeled in a chain with followUpQuestionnaires. But that' not enough as it doesn't allow for specialization per Organization, service, etc... So we'll need to change it at the CustomerEngagementQuestionnaire. Adding an order there, will be a bit tricky to do efficently, what makes the most sense would be to add it to the service's customerEngagementQuestionnaires relationship and makes that an array of custom types, like an index, or treat the array as an ordered collections that if re-ordered would have to treat as a full update to persist the same content at new indexes.
                                */
                                supplementalHealthQuestionnaire = supplementalInformedConsentQuestionnaire.followUpQuestionnaires[0],
                                supplementalHealthRespondentQuestionnairePromise;

                                supplementalInformedConsentRespondentQuestionnairePromise = this.createRespondentQuestionnaireIfNeeded(supplementalInformedConsentQuestionnaire)
                                .then((supplementalInformedConsentRespondentQuestionnaire) => {
                                    return (this.supplementalInformedConsentRespondentQuestionnaire = supplementalInformedConsentRespondentQuestionnaire);
                                });

                                supplementalHealthRespondentQuestionnairePromise = this.createRespondentQuestionnaireIfNeeded(supplementalHealthQuestionnaire)
                                .then((supplementalHealthRespondentQuestionnaire) => {
                                    return (this.supplementalHealthRespondentQuestionnaire = supplementalHealthRespondentQuestionnaire);
                                });
                            return Promise.all([supplementalInformedConsentRespondentQuestionnairePromise, supplementalHealthRespondentQuestionnairePromise])
                            .then(() => {
                                return supplementalInformedConsentQuestionnaire;
                            });
                        })
                    } else {
                        return null;
